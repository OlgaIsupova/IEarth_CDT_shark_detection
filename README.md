...
This is a group project for deep learning course at Intelligent Earth CDT. The project is on detecting sharks in underwater videos (Baited Remote Underwater Video Systems (BRUVS)).

## Goal of the project
The goal of the project is ideally to track individual sharks across frames and classify their species. The minimum is to highlight all periods of time with sharks present to allow a human annotator to skip across frames with nothing in them. 
...
* [Target video recordings](https://1drv.ms/f/c/39bbd3f227131a19/IgBdspyRoyk3SKucflmmnsvyAcuzobae3NwgjYjds_a1ZOE?e=5%3ae9BC9Y&sharingv2=true&fromShare=true&at=9) - BRUVS recordings. Unlabelled. The interesting videos to focus on are:
    * SALOU2411-001 (R or L, no preference)
    * DG2411-004R
    * VB2410-001 (R or L, no preference)

* Public data - Roboflow shark species dataset:
    * [Detection](https://drive.google.com/file/d/1IBDzvWqLaDSAdGNR0IyhaPhIAC9RzCpL/view?usp=sharing) - dataset for object detection (shark species classes) from roboflow. Each image has bounding box for a shark and it's species. 
...
* Any other publicly available data if you can find. The focus of species classification should be on the following species:
    * Grey reef shark
    * Black tip reef shark
    * White tip reef shark
    * Tawney nurse shark

* It may be beneficial to do a quick labelling yourself of the BRUVS recordings. If you want to explore this, there is this useful labelling [tool](https://www.robots.ox.ac.uk/~vgg/software/via/) from Oxford. For example, to run a model on one target video and manually correct its' predictions and add these corrected predictions back to training data. Doing this even with small number of corrections can bring a huge boost in performance on the rest of the videos.  
...
The code would ask whether the video is split into chapters. As you can see from out BRUVS videos, each long video is split into chapters. I have never run it on several chapters, only one chapter at a time, so I choose "N". 

I run the analysis mode first (without --peak parameter). In this mode sharktrack not only detects sharks but also tracks them across frames. In some videos it fails to track (outputs 0 tracks found), but I then run it again on the same video with --peek parameter, and it detects sharks, just independently on each frame without linking them across frames (because it fails to detect on each frame).  
...
* input_videos - the same folder with a video, which sharktrack uses (assumes one video)
...
* [Villon et al. BRUVS detector + maxN calculator](https://www.sciencedirect.com/science/article/pii/S1574954124000414#da0005) - BRUVS videos, detects and identifies shark species in both fully-automated and semi-automated fashion, computes maxN. Data (any) should be available upon request, requested with no reply so far.
